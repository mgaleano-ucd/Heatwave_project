...
table1_r1_jul2_sel <-table1_r1_jul2 %>%
  select(date, hhmmss, BH445_LEAF, BH445_BLOCK, BH445_VINE, E, A, Ci, gsw, Fv.Fm, Tleaf) %>%
  mutate(date = "07-01-2019", day = 182, year =2019, round =1)
table1_r1_jul2_sel <- table1_r1_jul2_sel [-1, ]
table2_r1_jul2_sel <-table2_r1_jul2 %>%
  select(date, hhmmss, BH455_leaf, BH455_BLOCK, BH455_VINE, E, A, Ci, gsw, Fv.Fm, Tleaf) %>%
  mutate(date = "07-01-2019", day = 182, year =2019, round =1)
...
table_comb__r2_Jul2 <- rbind (table1_r2_jul2_sel, table2_r2_jul2_sel, table3_r2_jul2_sel)
...
#Combining all rounds fro diurnal jul 2

names(table_comb__r1_Jul2) <- names(table_comb__r2_Jul2) 
names(table_comb__r2_Jul2) <- names(table_comb_r3_Jul2)
names(table_comb_r3_Jul2) <- names(table_comb_r4_Jul2)
names(table_comb_r4_Jul2) <- names(table_comb_r5_Jul2)

identical(names(table_comb__r1_Jul2), names (table_comb__r2_Jul2))
identical(names(table_comb__r2_Jul2), names (table_comb_r3_Jul2))
identical(names(table_comb_r3_Jul2), names (table_comb_r4_Jul2))
identical(names(table_comb_r4_Jul2), names (table_comb_r5_Jul2))

table_diurnals_Jul2 <-rbind(table_comb__r1_Jul2, table_comb__r2_Jul2, table_comb_r3_Jul2, table_comb_r4_Jul2, table_comb_r5_Jul2)
...
table1_midday_jul2 <- read.csv("data/2019-07-05-1200_BH_midday.csv", header = TRUE, sep =",", skip = 13)

table1_midday_jul2_sel <-table1_midday_jul2 %>%
  select (date, hhmmss, BH455_leaf, BH455_BLOCK, BH455_VINE, E, A, Ci, gsw, Fv.Fm, Tleaf) %>%
  mutate(date = "07-05-2019", day = 186, year =2019, round = "midday")
table1_midday_jul2_sel  <- table1_midday_jul2_sel  [-1, ]

write.csv(table1_midday_jul2_sel,"data_output/table_midday_jul5")
...
table2_r1_jul12_sel <-table2_r1_jul12 %>%
  select(date, hhmmss,BH_Leaf , BH_Block, BH_Vine, E, A, Ci, gsw, Fv.Fm, Tleaf) %>%
  mutate(date = "07-12-2019", day = 193, year =2019, round =1)
...
names(table_comb_r5_Jul12) <- names(table_comb_r6_Ju112)
...
write.csv(diurnals_before_blockid_change, "data_output/diurnals_before_blockid_change")
